"""
MyStudyHub Key Bypass — Playwright Standalone Version
======================================================
Same Two-Phase flow as mystudyhub_bypass.py:

  Phase 1: Visit arolinks.com/ALIAS (8s wait for CF cookies — csrfToken + AppSession)
  Phase 2: Navigate SAME browser context to bcsakhi.in (cookies carry over!)
           → inject intnt_io cookie → wait for #btn7 → JS click → token!

Usage:
  python mystudyhub_playwright.py
  python mystudyhub_playwright.py --visible
  python mystudyhub_playwright.py --visible --chrome
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
BASE     = "https://www.mystudyhub.shop"
...
async def bypass(arolinks_url: str, headless: bool = True, use_chrome: bool = False):
    ...
    async with async_playwright() as pw:
        ...
        # ── Click #btn7 (JS click first, direct click fallback) ──────────────
        if btn7_found:
            log("Clicking #btn7…")
            try:
...
def main():
    parser = argparse.ArgumentParser(description="MyStudyHub Bypass — Playwright")
    parser.add_argument("--visible", action="store_true", help="Show browser window")
    parser.add_argument("--chrome",  action="store_true", help="Use Chrome instead of Chromium")
    args = parser.parse_args()

    print(f"\n{C}{B}[MyStudyHub Bypass — Playwright Two-Phase Flow]{X}\n")
