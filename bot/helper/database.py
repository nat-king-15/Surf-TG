...
class Database:
    ...
    async def save_user_data(self, user_id, key, value):
    ...
    async def get_user_data_key(self, user_id, key, default=None):
        try:
            user_data = await self.db.users.find_one({"user_id": int(user_id)})
            if user_data:
    ...
    async def delete_user_data_key(self, user_id, key):
        try:
            await self.db.users.update_one(
                {"user_id": user_id},
                {"$unset": {key: ""}}
    ...
    async def get_user_session(self, user_id):
        return await self.get_user_data_key(user_id, 'session_string')

    async def remove_user_session(self, user_id):
        try:
            await self.db.users.update_one(
                {"user_id": user_id},
    ...
    async def save_user_bot(self, user_id, bot_token):
        from datetime import datetime
        try:
            await self.db.users.update_one(
                {"user_id": user_id},
    ...
    async def add_premium_user(self, user_id, duration_value, duration_unit):
        ...
        try:
            ...
            # Note: This checks every time, ideally should be done once on startup
            try:
                await self.db.premium_users.create_index("expireAt", expireAfterSeconds=0)
            except Exception:
                pass
    ...
    async def get_premium_details(self, user_id):
        try:
            user = await self.db.premium_users.find_one({"user_id": user_id})
            if user and "subscription_end" in user:
                return user
